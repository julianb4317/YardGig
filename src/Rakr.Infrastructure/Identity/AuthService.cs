...
using Rakr.Application.Auth.Models;
...
public class AuthService(
    ...
    Rakr.Application.Common.Interfaces.IAppDbContext appDb
) : IAuthService
{
    ...
    public async Task<Result<AuthResponse>> RegisterAsync(RegisterModel model, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                appDb.CustomerProfiles.Add(new Rakr.Domain.Entities.CustomerProfile
                {
                    UserId = user.Id
                });
            }
            ...
            {
                appDb.VendorProfiles.Add(new Rakr.Domain.Entities.VendorProfile
                {
                    UserId = user.Id,
                    VerificationStatus = Rakr.Domain.Enums.VerificationStatus.Approved // Auto-approve in dev
                });
            }
    ...
    public async Task<Result<MfaSetupResponse>> GetMfaSetupInfoAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        var qrCodeUri = $"otpauth://totp/Rakr:{UrlEncoder.Default.Encode(email!)}?secret={unformattedKey}&issuer=Rakr&digits=6";
